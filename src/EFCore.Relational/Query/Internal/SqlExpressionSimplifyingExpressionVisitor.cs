    ...
    public class SqlExpressionSimplifyingExpressionVisitor : ExpressionVisitor
    {
        ...
        private Expression SimplifySqlBinary(SqlBinaryExpression sqlBinaryExpression)
        {
                 ...
                 || sqlBinaryExpression.OperatorType == ExpressionType.OrElse)
            {
                ...
                {
                    var leftConstantIsEnumerable = leftCandidateInfo.constantExpression.Value is IEnumerable
                        && !(leftCandidateInfo.constantExpression.Value is string)
                        && !(leftCandidateInfo.constantExpression.Value is byte[]);

                    var rightConstantIsEnumerable = rightCandidateInfo.constantExpression.Value is IEnumerable
                        && !(rightCandidateInfo.constantExpression.Value is string)
                        && !(rightCandidateInfo.constantExpression.Value is byte[]);
                    ...
                    else if (leftConstantIsEnumerable && rightConstantIsEnumerable)
                    {
                        ...
                        var resultArray = IntersectCollections(
                            (IEnumerable)leftCandidateInfo.constantExpression.Value,
                            (IEnumerable)rightCandidateInfo.constantExpression.Value);
